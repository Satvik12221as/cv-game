...
class SplashEffect:
    def __init__(self, pos, color, num_particles=30, duration=0.8, gravity=400):
        ...
        for _ in range(num_particles):
            ...
            self.particles.append({
                'pos': list(pos),  # Use a mutable list for position
                'vel': [vx, vy],
                'radius': random.uniform(3, 8),
                'initial_radius': random.uniform(3, 8)
            })
    ...
    def update(self, dt):
        ...
        for p in self.particles:
            ...
            p['radius'] = p['initial_radius'] * (self.timer / self.duration)

    def draw(self, surface):
        # Calculate alpha once for all particles
        progress = self.timer / self.duration
        alpha = int(255 * (progress ** 2)) # Use ease-in for a more natural fade

        for p in self.particles:
            if p['radius'] > 0:
                # Create a color tuple with the calculated alpha
                current_color = self.color + (alpha,)
                pos = (int(p['pos'][0]), int(p['pos'][1]))
                radius = int(p['radius'])
                # Draw directly onto the main surface for better performance
                pygame.draw.circle(surface, current_color, pos, radius)
